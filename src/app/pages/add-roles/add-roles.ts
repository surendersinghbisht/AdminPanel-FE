...
preventConsecutiveSpaces(event: KeyboardEvent, controlName: string) {
  ...
  // Prevent space if:
  // 1. Previous character is a space (consecutive space)
  // 2. Input is empty (leading space)
        ...
        next: (response: any) => {
          ...
          this.roleForm.patchValue({
            ...
            permissions: response.permission.map((perm: any) => {
             const name = this.toPascalCase(perm.name || perm.permissionName || 'Unknown Permission');
